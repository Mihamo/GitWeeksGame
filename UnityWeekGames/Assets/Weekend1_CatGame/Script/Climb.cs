...
public class Climb : MonoBehaviour
{
    ...
    public bool CheckForClimb(float speedbonus)
    {
        ...
        if (Physics.Raycast(origin, dir, out hit, 0.52f))
        {
    ...
    public void LookForGround(Vector3 dirMove)
    {
        Vector3 origin = transform.position;
        Vector3 dir = -Vector3.up;
        RaycastHit hit;
        if (Physics.Raycast(origin, dir, out hit, 1.1f) && dirMove == dir)
        {
            StopClimb();
    ...
    bool CanMove(Vector3 moveDir)
    {
        ...
        Debug.DrawRay(origin, dir, Color.blue, 10f);
        if (Physics.Raycast(origin, dir, out hit, dis))
        {
            helper.position = PosWithOffset(origin, hit.point);
